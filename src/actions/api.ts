...
export const addAnAPIReview = async(reviewData:reviewCreation)=>{
    try {
        ...
        const success = await basedApiUrl.post(`/userApi/createReview/${reviewData.apiId}`,{
...
export const reportAnAPI=async(formData:any,api_id:number,user_id:number)=>{
    try {
        const response = await basedApiUrl.post(`/userApi/reportAPI/${api_id}`,JSON.stringify({...formData,userId:user_id}),{
            headers:{
                "Content-Type":"application/json"
            }
        })
...
export const reportAnComment = async(reason:string,description:string,commentId:string,userId:number)=>{
    try {
        const response = await basedApiUrl.post(`/userApi/reportComment/${commentId}`,JSON.stringify({
            reason,description,userId
        }),{
            headers:{
                "Content-Type":"application/json"
            }
...
export const deleteReview=async (id:number)=>{
    try {
        const isDeleted = await basedApiUrl.get(`/userApi/deleteReviewReport/${id}`)
        return isDeleted
    } catch (error) {
        console.log(error)
    }
